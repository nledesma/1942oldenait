...
void Servidor::pasivar() {
    const char* path = this->getEscenario()-> getFondoSprite();
    ...
    while (servidor->servidorActivo()) {
        try {
            ...
            if (servidor->hayLugar()){
                ...
                cout << "Clientes conectados: " << servidor->hayClientesConectados() << endl;
            } else {
...
void Servidor::agregarCliente(int fdCliente) {
    ...
    if(hayClientesConectados() > 0){
        this->getEscenario()->iniciar("galaxia2.bmp");
    }
...
int Servidor::hayClientesConectados(){
